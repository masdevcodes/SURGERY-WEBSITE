...
export function Services() {
  ...
  // ✅ **MODIFICATION:** Function to dynamically collect all images from your data
  const getAllImagePaths = useCallback(() => {
    // 1. Collect all banner images from the 'services' array
    const bannerImages = services.map(service => service.banner);
    
    // 2. Combine with other static images (carousel, background, etc.)
    const allPaths = [
      ...bannerImages,
      ...carouselImages,
      '/111.webp',
    ...
    // 3. Use a Set to remove any duplicates and return the unique list
    return [...new Set(allPaths)];
  }, [services, carouselImages]);

  // The 'allImages' constant is now dynamically generated by the function
  ...
  }, [allImages.length]); // Depend on allImages.length to re-run if it changes
  ...
  return (
    <>
      {/* 🔥 CRITICAL: HIDDEN PRELOAD IMAGES - This now uses the dynamic 'allImages' list */}
