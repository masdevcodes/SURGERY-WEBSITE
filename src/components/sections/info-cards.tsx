...
export function InfoCards() {
  ...
  // Fixed Modal Component - Moved outside to avoid re-rendering issues
  const Modal = ({
    children,
    onClose,
  }: {
    ...
    >
      ...
      >
        <button
          ...
          className="absolute top-4 right-4 z-10 text-gray-600 hover:text-gray-900 text-3xl font-bold"
          onClick={onClose}
        >
          &times;
        </button>
  ...
  // Fixed Modal Content Function - Cleaner implementation
  function getModalContent(key: string) {
    const contentMap = {
      stoma: {
        title: "Stoma Clinic Details",
        image: "/images/infocard/sto2.webp",
      ...
      <div className="flex flex-col">
        ...
        <div className="w-full h-64 relative">
          <OptimizedImage
            ...
            priority={loadedImages.has(content.image)}
            onLoad={() => handleImageLoad(content.image)}
          />
  ...
  const clinicCards = {
    ...
    slider: {
      title: "OPD Services",
      description: "Our outpatient department provides comprehensive medical services with state-of-the-art facilities.",
      images: sliderImages,
  ...
  const renderClinicCard = (key: string) => {
          ...
          <div className="relative overflow-hidden flex-grow">
            <div className="w-full h-full relative">
              <OptimizedImage
                ...
                alt={`${clinic.title} image ${clinic.imageIndex + 1}`}
                fill
    ...
    return (
      <div 
        key={key} 
        className="rounded-lg shadow-lg overflow-hidden flex flex-col h-full group hover:shadow-xl transition-all duration-500 ease-in-out transform hover:-translate-y-1 cursor-pointer"
      ...
      >
        ...
        <div className="p-8 bg-white flex-grow flex flex-col justify-between">
          <div>
            <h3 className="text-3xl font-bold text-blue-950 mb-4 group-hover:text-teal-500 transition-colors duration-500 text-center">
              {clinic.title}
            </h3>
          ...
          <div className="text-center">
            <span className="font-bold text-teal-500 flex items-center gap-2 justify-center hover:text-teal-600 transition-colors duration-300">
